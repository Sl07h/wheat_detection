...
{
    (Invoke-WebRequest -Uri $URL -SessionVariable googleDriveSession).Links.Href > qwe.txt
    ...
    $DownloadURL
    Invoke-WebRequest -Uri $DownloadURL -OutFile "arch.zip" -WebSession $googleDriveSession
    7z x arch.zip
    rm arch.zip
    rm qwe.txt
}
